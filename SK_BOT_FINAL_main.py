...
async def make_signal(symbol, timeframe="1min", mode="REAL"):
    ...
    if buy >= CONSENSUS_REQUIRED and buy > sell and not conflict:
        action = "BUY"
    elif sell >= CONSENSUS_REQUIRED and sell > buy and not conflict:
        action = "SELL"
    else:
        action = "WAIT"
    ...
    return {
        ...
        "action": action, "buy_votes": buy, "sell_votes": sell,
        "neutral_votes": neutral, "buy_weight": buy_weight,
        "sell_weight": sell_weight,
        "weighted_consensus": max(buy_weight, sell_weight),
        "required_votes": CONSENSUS_REQUIRED, "news": news,
        "price": price, "entry_price": price,
        "entry_time_utc6": entry.strftime("%Y-%m-%d %H:%M:%S"),
        "expiry_time_utc6": expiry.strftime("%Y-%m-%d %H:%M:%S"),
        "timezone": "UTC+6", "updated_at": now.isoformat(),
        "strategies": votes,
    }
...
<title>SK BOT</title>
<style>
body{margin:0;background:#080d19;color:#eef2ff;font-family:Arial}
.wrap{max-width:560px;margin:auto;padding:18px}.header{display:flex;align-items:center;gap:12px}
.logo{width:56px;height:56px;border-radius:16px;background:linear-gradient(135deg,#6d5dfc,#00d4ff);display:flex;align-items:center;justify-content:center;font-weight:900}
h1{margin:0}.muted,.small{color:#98a4c0;font-size:12px}.card{background:#11192c;border:1px solid #273352;border-radius:20px;padding:16px;margin-top:15px}
label{display:block;color:#aab6d2;font-size:12px;margin:8px 0}
select,button{width:100%;box-sizing:border-box;padding:13px;border-radius:12px;margin-bottom:10px;font-size:15px}
select{background:#080f20;color:#fff;border:1px solid #2b395d}button{background:#6d5dfc;color:#fff;border:0;font-weight:800}
.grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}.stat{background:#080f20;border-radius:14px;padding:12px;text-align:center}
.big{font-size:25px;font-weight:900;margin:5px 0}.buy{color:#4ade80}.sell{color:#fb7185}.wait{color:#facc15}
.row{display:flex;justify-content:space-between;padding:9px 0;border-bottom:1px solid #222c45}.hidden{display:none}
.badge{display:inline-block;padding:5px 9px;border-radius:999px;background:#1b2b49;color:#9fd7ff;font-size:11px;font-weight:700}.otc{color:#facc15}
</style></head><body><div class="wrap">
<div class="header"><div class="logo">SK</div><div><h1>SK BOT</h1><div class="muted">25-Strategy AI Market Scanner</div></div></div>
<div class="card">
<label>Market Mode</label><select id="mode"><option value="REAL">REAL FX</option><option value="OTC">OTC</option></select>
<label>Market</label><select id="market"></select>
<label>Timeframe</label><select id="timeframe">
...
<option value="60min">1 HOUR</option></select>
<button onclick="getSignal()">GET SIGNAL</button></div>
<div class="card hidden" id="result">
<div class="muted">CURRENT SIGNAL <span id="modeBadge" class="badge">REAL FX</span></div>
<div id="action" class="big wait">WAIT</div>
<div class="grid"><div class="stat"><div class="small">BUY</div><div id="buy" class="big buy">0/25</div></div>
<div class="stat"><div class="small">SELL</div><div id="sell" class="big sell">0/25</div></div></div>
<div class="grid" style="margin-top:10px"><div class="stat"><div class="small">NEUTRAL</div><div id="neutral" class="big wait">0/25</div></div>
<div class="stat"><div class="small">WEIGHT</div><div id="weight" class="big">0%</div></div></div>
<div class="row"><span>News</span><b id="news">NEUTRAL</b></div>
<div class="row"><span>Entry Price</span><b id="price">-</b></div>
<div class="row"><span>Timeframe</span><b id="tf">-</b></div>
<div class="row"><span>Entry UTC+6</span><b id="entryTime">-</b></div>
<div class="row"><span>Expiry UTC+6</span><b id="expiryTime">-</b></div>
<p class="small">Weight is model importance, not guaranteed accuracy or profit.</p></div>
<div class="card"><div class="small"><b>14 / 25 CONSENSUS</b></div>
<p class="small">BUY/SELL is issued only when at least 14 strategies agree and news does not conflict.</p>
<p class="small otc">OTC requires a connected OTC price feed. No fake OTC price/signal is generated.</p></div>
</div>
<script>
async function loadMarkets(){const r=await fetch("/markets");const d=await r.json();d.markets.forEach(s=>market.add(new Option(s,s)))}
function tfLabel(tf){return tf==="60min"?"1 HOUR":tf.replace("min"," MIN")}
async function getSignal(){
 const r=await fetch("/signal?symbol="+encodeURIComponent(market.value)+"&timeframe="+encodeURIComponent(timeframe.value)+"&mode="+encodeURIComponent(mode.value));
 const d=await r.json();if(d.detail){alert(d.detail);return}
 result.classList.remove("hidden");action.textContent=d.action;
 action.className="big "+(d.action==="BUY"?"buy":d.action==="SELL"?"sell":"wait");
 buy.textContent=d.buy_votes+"/25";sell.textContent=d.sell_votes+"/25";neutral.textContent=d.neutral_votes+"/25";
 weight.textContent=d.weighted_consensus+"%";news.textContent=d.news.sentiment;price.textContent=d.entry_price;
 tf.textContent=tfLabel(d.timeframe);entryTime.textContent=d.entry_time_utc6;expiryTime.textContent=d.expiry_time_utc6;
 modeBadge.textContent=d.mode==="OTC"?"OTC":"REAL FX"}
loadMarkets();
</script></body></html>"""
...
async def startup():
    print("SK BOT started | 25 strategies | 14/25 consensus | 1m-1h timeframes")
    if TELEGRAM_TOKEN:
