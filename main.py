...
logger = logging.getLogger("SK_BOT_META5")# SK BOT PRO ULTRA — BINARY OPTIONS ENGINE (META-5 HIGH-ACCURACY BUILD)
import os
import asyncio
import sqlite3
import json
...
async def fetch_real_candles(market: str):
    ...
    domain_name = "://twelvedata.com"
    endpoint_path = "time_series"
    url = f"https://{domain_name}/{endpoint_path}?symbol={api_symbol}&interval=1min&outputsize=65&apikey={TWELVE_DATA_API_KEY}"
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            logger.error(f"API Connection issue for {market}: {e}")
            pass
...
async def send_telegram_alert(market, signal, score, price):
    ...
    telegram_bot_path = f"bot{TELEGRAM_TOKEN}"
    url = f"https://{telegram_domain}/{telegram_bot_path}/sendMessage"
...
async def dashboard():
    options_html = ""
    for k, v in MARKET_CATALOG.items():
        options_html += f'<option value="{k}">{v["name"]}</option>'
