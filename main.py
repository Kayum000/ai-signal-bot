...
async def get_fx_data(symbol: str) -> pd.DataFrame:
    ...
    if not ALPHA_VANTAGE_API_KEY:
        raise HTTPException(500, "Add ALPHA_VANTAGE_API_KEY first.")
...
h1{margin:0}.muted,.small{color:#98a4c0;font-size:12px}
.card{background:#11192c;border:1px solid #273352;border-radius:20px;padding:16px;margin-top:15px}
label{display:block;color:#aab6d2;font-size:12px;margin:8px 0}
select,input,button{width:100%;box-sizing:border-box;padding:13px;border-radius:12px;margin-bottom:10px;font-size:15px}
select,input{background:#080f20;color:#fff;border:1px solid #2b395d}
button{background:#6d5dfc;color:#fff;border:0;font-weight:800}
.grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}
.stat{background:#080f20;border-radius:14px;padding:12px;text-align:center}
.big{font-size:25px;font-weight:900;margin:5px 0}.buy{color:#4ade80}.sell{color:#fb7185}.wait{color:#facc15}
.row{display:flex;justify-content:space-between;padding:9px 0;border-bottom:1px solid #222c45}
.hidden{display:none}
</style>
</head>
<body>
...
<div class="muted">CURRENT SIGNAL</div>
<div id="action" class="big wait">WAIT</div>
<div class="grid">
<div class="stat"><div class="small">BUY</div><div id="buy" class="big buy">0/25</div></div>
<div class="stat"><div class="small">SELL</div><div id="sell" class="big sell">0/25</div></div>
</div>
<div class="grid" style="margin-top:10px">
<div class="stat"><div class="small">NEUTRAL</div><div id="neutral" class="big wait">0/25</div></div>
<div class="stat"><div class="small">WEIGHT</div><div id="weight" class="big">0%</div></div>
</div>
<div class="row"><span>News</span><b id="news">NEUTRAL</b></div>
<div class="row"><span>Price</span><b id="price">-</b></div>
<div class="row"><span>Timeframe</span><b>1 MIN</b></div>
<p class="small">Weight is model importance, not guaranteed accuracy or profit.</p>
</div>

<div class="card">
<div class="small"><b>14 / 25 CONSENSUS</b></div>
...
async function loadMarkets(){
 const r=await fetch("/markets"); const d=await r.json();
 d.markets.forEach(s=>market.add(new Option(s,s)));
}
async function getSignal(){
 const s=market.value;
 const r=await fetch("/signal?symbol="+encodeURIComponent(s));
 ...
 buy.textContent=d.buy_votes+"/25";
 sell.textContent=d.sell_votes+"/25";
 neutral.textContent=d.neutral_votes+"/25";
 weight.textContent=d.weighted_consensus+"%";
 news.textContent=d.news.sentiment;
...
@app.get("/signal")
async def signal(symbol: str = "EURUSD"):
    return await make_signal(symbol)
